...
public class LivraisonClientController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
         List<Livraison> blogs = new ArrayList();
        blogs.addAll(livraisonService.GetLivraison(1));
        if(blogs.size() > 0){
            setChosenLivraison(blogs.get(0));
            myListener = new MyListenerLivraison() {
                @Override
                public void onClickListener(Livraison livraison) {
                    setChosenLivraison(livraison);
                }
            };
    }
    int column =0;
        ...
        for(int i=0; i <blogs.size(); i++){
            ...
            fxmlLoader.setLocation(getClass().getResource("../GUI/Livraison.fxml"));
            AnchorPane anchorPane = fxmlLoader.load();
            LivraisonController livraisonController = fxmlLoader.getController();
            
            livraisonController.setData(blogs.get(i),myListener);
            if(column == 3){
                column=0;
                row++;
            }
    ...
    private void setChosenLivraison(Livraison livraison){
    client.setText(livraison.getUser().getPrenom()+" "+livraison.getUser().getNom());
    statut.setText(livraison.getStatut());
    date.setText(""+livraison.getDate_livraison());
    adresse.setText(livraison.getAdresse());
    total.setText(livraison.getTotal()+"");
}    
